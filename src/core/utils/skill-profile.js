...
const createSkillProfileUtils = (SkillCategory) => {
    ...
    const updateSkillProfile = (profile, eventId, minutes) => {
        if (!eventId) return;
        if (profile && typeof profile.apply_practice_event === 'function') {
            try {
                profile.apply_practice_event(eventId, minutes);
                return;
            } catch {
                // Fall back to JS rules if WASM method isn't ready yet.
            }
        }
