...
const tryWasmSeed = async (skillSamples, songSamples) => {
...
const computeRecommendations = async () => {
    ...
    const skillSamples = buildSkillSamples(adaptiveLog);
    const songSamples = buildSongSamples(recentSongEvents);
    const wasmSeed = await tryWasmSeed(skillSamples, songSamples);
