...
const buildSkillSamples = (adaptiveLog) => adaptiveLog
    .map((entry) => {
...
const buildSongSamples = (songEvents) => songEvents.map((event) => ({
    accuracy: clamp(event.accuracy || 0, 0, 100),
    timestamp: Number.isFinite(event.timestamp) ? event.timestamp : null,
}));

export const computeRecommendationsFromData = async ({
    events = [],
    adaptiveLog = [],
    metronomeTuning = {},
    computeSeed = null,
    ...
    const adaptiveList = Array.isArray(adaptiveLog) ? adaptiveLog : [];
    const songEvents = eventList.filter((event) => event?.type === 'song');
    const recentSongEvents = songEvents.slice(-8);
    const skillSamples = buildSkillSamples(adaptiveList);
    const songSamples = buildSongSamples(recentSongEvents);

    let wasmSeed = null;
    if (typeof computeSeed === 'function') {
        try {
            wasmSeed = await computeSeed(skillSamples, songSamples);
        } catch {
            wasmSeed = null;
        }
    }

    let weakestSkill = wasmSeed?.weakestSkill;
    let skillScores = wasmSeed?.skillScores;
    let songLevel = wasmSeed?.songLevel;

    const skillTotals = new Map();
    const skillCounts = new Map();

    if (!weakestSkill || !skillScores || !songLevel) {
        adaptiveList.forEach((entry) => {
            const skill = SKILL_BY_GAME[entry.id];
            if (!skill) return;
            const value = clamp(Number.isFinite(entry.accuracy) ? entry.accuracy : entry.score || 0, 0, 100);
        ...
        weakestSkill = skillCandidates.reduce((weakest, skill) => {
        ...
        songLevel = averageSong >= 85 ? 'advanced' : averageSong >= 65 ? 'intermediate' : 'beginner';
    }
