import initCore, {
    ...
    SkillCategory,
    calculate_streak,
} from '@core/wasm/panda_core.js';
import { getJSON, setJSON, removeJSON } from '@core/persistence/storage.js';
import { createSkillProfileUtils } from '@core/utils/skill-profile.js';
...
const weekTargetEl = document.querySelector('[data-progress="week-target"]');
const weekTrackEl = document.querySelector('[data-progress="week-track"]');
...
const parentChartPointEls = Array.from(document.querySelectorAll('[data-parent="week-points"] circle'));
...
const recentGameCards = Array.from(document.querySelectorAll('[data-recent-game]')).map((el) => ({
    el,
    titleEl: el.querySelector('[data-recent-game-title]'),
    scoreEl: el.querySelector('[data-recent-game-score]'),
}));

const achievementItems = Array.from(document.querySelectorAll('[data-achievement]')).map((el) => ({
    el,
    id: el.dataset.achievement,
}));
...
const pathNodes = Array.from(document.querySelectorAll('.path-node[data-path-level]')).map((node) => {
    const required = Number.parseInt(node.dataset.pathLevel || '1', 10);
    return { node, required: Number.isNaN(required) ? 1 : required };
});
...
const setLockedState = (el, locked) => {
    if (!el) return;
    if (locked) {
...
const updateUI = ({ progress, tracker, streak, weekMinutes, dailyMinutes, skills, weakestSkill, recentGames }) => {
    ...
    if (weekTargetEl || weekTrackEl) {
        ...
        updateProgressTrack(weekTrackEl, percent, `${weekMinutes} of ${weeklyTarget} minutes`);
    }

    const goalTarget = getDailyGoalTarget();
    if (dailyGoalValueEl) dailyGoalValueEl.textContent = String(goalTarget);
    if (dailyGoalTrackEl && Array.isArray(dailyMinutes)) {
        const todayMinutes = dailyMinutes[dailyMinutes.length - 1] || 0;
        const percent = clamp(Math.round((todayMinutes / Math.max(1, goalTarget)) * 100), 0, 100);
    ...
    if (Array.isArray(dailyMinutes)) {
        ...
        if (parentChartPointEls.length) {
            parentChartPointEls.forEach((circle, index) => {
                const point = points[index];
                if (!point) {
                    circle.setAttribute('data-hidden', 'true');
                    return;
                }
                circle.removeAttribute('data-hidden');
    ...
    achievementItems.forEach(({ el, id }) => {
        if (!id) return;
        const unlocked = tracker.is_unlocked(id);
        setLockedState(el, !unlocked);
    });

    if (recentGameCards.length) {
        const hasGames = Array.isArray(recentGames) && recentGames.length > 0;
        recentGameCards.forEach(({ el, titleEl, scoreEl }, index) => {
    ...
    pathNodes.forEach(({ node, required }) => {
        if (!Number.isFinite(required)) return;
        setLockedState(node, progress.level < required);
    });
