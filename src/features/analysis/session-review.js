import initCore, { SkillProfile, SkillCategory } from '@core/wasm/panda_core.js';
import { getLearningRecommendations } from '@core/ml/recommendations.js';
import { getJSON, getBlob } from '@core/persistence/storage.js';
...
const chartPointEls = chartPoints ? Array.from(chartPoints.querySelectorAll('circle')) : [];
const chartCaption = document.querySelector('[data-analysis="chart-caption"]');
const chartCard = document.querySelector('.analysis-chart');
...
const setChartEmpty = (message = 'No sessions yet. Play a song to see your trend.') => {
    if (chartCard) chartCard.dataset.empty = 'true';
    if (chartLine) chartLine.setAttribute('d', 'M20 90 L300 90');
    chartPointEls.forEach((circle) => {
        circle.setAttribute('data-hidden', 'true');
    });
    if (chartCaption) chartCaption.textContent = message;
};

const clearChartEmpty = () => {
    if (chartCard) delete chartCard.dataset.empty;
};
...
const updateChart = (values) => {
    ...
    chart.points.forEach((point, index) => {
        const circle = chartPointEls[index];
        if (!circle) return;
    ...
    for (let i = chart.points.length; i < chartPointEls.length; i += 1) {
        chartPointEls[i].setAttribute('data-hidden', 'true');
    }
