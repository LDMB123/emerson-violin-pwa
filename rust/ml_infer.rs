...
#[cfg(feature = "ml-onnx")]
struct TractModel {
  plan: Rc<SimplePlan<TypedFact, Box<dyn TypedOp>, TypedModel>>,
}
...
pub fn run_active_inference(state: Rc<RefCell<AppState>>, entry: &ModelEntry) {
  let entry = entry.clone();
  spawn_local(async move {
    let ml_state = state.borrow().ml.clone();
    let result = run_inference(&entry, &ml_state).await;
    match result {
      Some(output) => {
...
fn run_tract(id: &str, input_len: usize, output_len: usize, bytes: &[u8], features: &[f32]) -> Option<Vec<f32>> {
  let cached = TRACT.with(|cache| cache.borrow().get(id).map(|model| Rc::clone(&model.plan)));
  ...
  let result = plan.run(tvec!(input.into_tvalue())).ok()?;
  let tensor = result.first()?;
