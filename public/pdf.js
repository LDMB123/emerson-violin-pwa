...
function toMessage(error) {
  ...
  try { return String(error); } catch { return 'PDF error'; }
}
...
async function load(blob) {
  ...
  try {
    ...
    const hint = (navigator && navigator.onLine === false)
      ? ' (offline: cache the PDF offline pack and retry)'
      : '';
    dispatch('pdf-error', { message: `${toMessage(error)}${hint}` });
    return false;
  }
}
...
async function renderPage(pageNumber, containerSelector = '[data-score-visual]') {
  ...
  try {
    ...
    dispatch('pdf-error', { message: toMessage(error) });
    return false;
  }
}
