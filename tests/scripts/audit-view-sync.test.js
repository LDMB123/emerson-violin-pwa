...
describe('audit-view-sync', () => {
    it('extracts inline view-home markup from index html', () => {
        const indexHtml = `
            <main>
                <section class="view" id="legacy-view-home"><div>Home</div></section>
                <section class="view" id="view-coach"><div>Coach</div></section>
            </main>
        `;

        const homeView = extractInlineHomeView(indexHtml);
        expect(homeView).toContain('id="legacy-view-home"');
        expect(homeView).toContain('<div>Home</div>');
    });

    it('normalizes equivalent markup with whitespace differences', () => {
        const compact = '<section class="view" id="legacy-view-home"><div>Home</div></section>';
        const formatted = `
            <section class="view" id="legacy-view-home">
                <div>Home</div>
            </section>
        `;
    ...
    it('reports home view sync parity correctly', () => {
        const inlineHome = '<section class="view" id="legacy-view-home"><h1>Ready</h1></section>';
        const sameRouteHome = `
            <section class="view" id="legacy-view-home">
                <h1>Ready</h1>
            </section>
        `;
        const changedRouteHome = '<section class="view" id="legacy-view-home"><h1>Different</h1></section>';
